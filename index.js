...
const totalGames = GAMES_JSON.reduce((accum, game) => {
    return accum + 1;
} , 0)
...
// we initially want to only show funded games
filterFundedOnly();
...
const displayStr = 
    `A total of $100,000 has been raised for 4 games. Currently, ${numUnfundedGames} game${numUnfundedGames == 1 ? "" : "s"} remain${numUnfundedGames != 1 ? "" : "s"} unfunded. We need your help to fund these amazing games!`
...
topGamePara.innerHTML = `<p>${firstGame.name}</p>`;
firstGameContainer.append(topGamePara);

// do the same for the runner up item
const secondGamePara = document.createElement("p");
secondGamePara.innerHTML = `<p>${secondGame.name}</p>`;
